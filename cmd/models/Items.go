package models

import "github.com/gofrs/uuid"

type Item struct {
	ID        string `gorm:"primary_key;"`
	TableName string `gorm:"-"`
	Caption   string
	Statuses  []ObjectStatus `gorm:"many2many:item_statuses;"`
}

func (item *Item) New(name string) {
	statuses := []ObjectStatus{
		{
			StatusGroupCode: "U_ACTIVATION_SG",
			StatusCode:      "NOT_ACTIVE",
		},
		{
			StatusGroupCode: "U_LIFECYCLE_SG",
			StatusCode:      "PREPARED",
		},
	}
	item.ID = uuid.Must(uuid.NewV4()).String()
	item.TableName = "items"
	item.Caption = name
	item.Statuses = statuses
}

func (item *Item) SetTable(table string) {
	item.TableName = table
}

func (item *Item) GetID() string {
	return item.ID
}

func (item *Item) GetCaption() string {
	return item.Caption
}

func (item *Item) GetStatuses() []ObjectStatus {
	return item.Statuses
}

func (item *Item) GetTable() string {
	return item.TableName
}

func (item *Item) GetStatus(caption string) ObjectStatus {
	for _, status := range item.Statuses {
		if status.StatusGroupCode == caption {
			return status
		}
	}
	return ObjectStatus{}
}

func (item *Item) SetStatus(caption string, stat string) {
	for i, status := range item.Statuses {
		if status.StatusGroupCode == caption {
			item.Statuses[i].StatusCode = stat
			return
		}
	}
	item.Statuses = append(item.Statuses, ObjectStatus{
		StatusGroupCode: caption,
		StatusCode:      stat,
	})
}
